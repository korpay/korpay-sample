...
import com.korpay.model.CancelRequest;
...
public class ExampleController {
    ...
    @GetMapping("/cancel")
    public String cancel(HttpServletResponse httpResponse, Model model) {
        httpResponse.setHeader("Cache-Control", "no-cache, no-store, must-revalidate");
        httpResponse.setHeader("Pragma", "no-cache");
        httpResponse.setHeader("Expires", "0");

        /*
            취소 요청 파라미터는 CancelRequest 를 확인해주세요.
         */
        model.addAttribute("cancel", new CancelRequest());
        return "cancel";
    }
}
